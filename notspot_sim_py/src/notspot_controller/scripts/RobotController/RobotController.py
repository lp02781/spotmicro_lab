...
class Robot(object):
    ...
    def joystick_command(self,msg):
        if msg.buttons[0]: # rest 			Triangle
            self.command.trot_event = False
            self.command.crawl_event = False
            self.command.stand_event = False
            self.command.rest_event = True
            
        elif msg.buttons[1]: # trot 		1 : Circle
            self.command.trot_event = True
            self.command.crawl_event = False
            self.command.stand_event = False
            self.command.rest_event = False

        elif msg.buttons[2]: # crawl 		2 : X
            self.command.trot_event = False
            self.command.crawl_event = True
            self.command.stand_event = False
            self.command.rest_event = False
       
        elif msg.buttons[3]: # stand 		3 : Rectangle
            self.command.trot_event = False
            self.command.crawl_event = False
    ...
    def imu_orientation(self,msg):
        ...
        self.state.imu_roll = roll#rpy_angles[0]
        self.state.imu_pitch = pitch#rpy_angles[1]
